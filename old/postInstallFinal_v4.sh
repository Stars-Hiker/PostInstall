...
install_qemu_kvm() {
    ...
    local pkgs=(
        ...
        tuned               # CPU/IO profile daemon (used in desktop power mode)
    )
...
install_power_management() {
    ...
    if [[ "$MACHINE_TYPE" == "laptop" ]]; then
        # TLP and tuned both compete for CPU governor control and will
        # conflict. Disable tuned first if it snuck in enabled.
        if service_enabled tuned.service; then
            warn "tuned is enabled — masking it to avoid conflict with TLP."
            sudo systemctl disable --now tuned.service || true
            sudo systemctl mask tuned.service || true
        fi
        ...
        # Desktop mode: use tuned with the "balanced" profile.
        # (tuned was installed in install_qemu_kvm.)
